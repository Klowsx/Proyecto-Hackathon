...
export default function ProductCard({ name, description, price, image }) {
  ...
  return (
    <div className="hover:scale-105 transform transition-transform duration-300 flex flex-col border w-64 h-96 border-gray-50 p-6 rounded-xl items-center justify-center">
      ...
      <img src={image} alt="image" className="w-30 h-36 rounded-lg" />
      <h1 className="text-lg font-bold text-center">{name}</h1>
      <p className="overflow-hidden h-24 ">{description}</p>
      <p className="mt-3">${price}</p>
      <button
        className="p-3 mt-1 bg-purple-700 rounded-lg w-24"
        onClick={() => {
          dispatch({
            type: "ADD_PRODUCT",
